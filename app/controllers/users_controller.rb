...
#    end
  ...
  def followings
    ...
    @followings = @user.followings.search(params[:search]).order('@ranking_counts desc').page(params[:page]).per(24)
    counts(@user)
  end
  
  def followers
    @ranking_counts = Follow.ranking
    @followers = @user.followers.search(params[:search]).order('@ranking_counts desc').page(params[:page]).per(24)
    counts(@user)
  end
